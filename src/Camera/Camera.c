...
void camera_inputs(Camera* cam, const Uint8* keys, int mouse_dx, int mouse_dy, int mouse_held) {
    ...
    if (keys[SDL_SCANCODE_W]) {
        glm_vec3_scale(cam->orientation, cam->speed, tmp);
    ...
    if (keys[SDL_SCANCODE_A]) {
        glm_vec3_cross(cam->orientation, cam->up, tmp);
    ...
    if (keys[SDL_SCANCODE_SPACE]) {
        glm_vec3_scale(cam->up, cam->speed, tmp);
        glm_vec3_add(cam->position, tmp, cam->position);
    }
    if (keys[SDL_SCANCODE_LSHIFT]) {
        glm_vec3_scale(cam->up, -0.1f, tmp);
        glm_vec3_add(cam->position, tmp, cam->position);
    }
